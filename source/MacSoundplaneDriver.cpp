  ...
  else if(mostRecentSeq != mPreviousSeq)
  {
    ...
    if (payloadsOK)
    {
      if(mStartupCtr >= kIsochStartupFrames)
      {
        ...
        rawPayloadsToFrame(payloads[0], payloads[1], mWorkingFrame);
        clearFrameEdges(mWorkingFrame);
        
        bool firstFrame = (mStartupCtr == kIsochStartupFrames);
        float diff = firstRowFrameDiff(mWorkingFrame, mPrevFrame);
